...
const DefaultTemplate: React.FC<{
  ...
  const error =
    schema.type !== "object" ? get(methods.formState.errors, fieldName) : "";
  /**
   * Note: when schema type is object, dont need to show error field
   */
  const errorMessage = error
    ? error?.message || `${label || "This field"} is required`
    : "";
  ...
  return (
    <div className={`mb-3 ${error ? "invalid" : "valid"}`}>
      ...
      <div>
        ...
        {(help || error) && (
          <div
            className={`help mt-1 d-block ${
              error ? "invalid-feedback" : "text-secondary"
            }`}
          >
            {errorMessage || help}
          </div>
...
const SchemaField = ({
  ...
  idSchema = mergeObjects(
    toIdSchema(schema, null, rootSchema, idPrefix, idSeparator),
    idSchema
  );
  ...
  const description =
    uiSchema["ui:description"] || rest.schema.description || schema.description;
  const help = uiSchema["ui:help"];
  const hidden = uiSchema["ui:widget"] === "hidden";

  const fieldProps = {
    description: (
      <DescriptionField description={description} formContext={formContext} />
    ),
    ...
    imageHint: (
      <ImageHint
        ...
        name={name}
        schema={schema}
        uiSchema={uiSchema["ui:imageHelper"]}
      />
    ),
    id,
    name,
